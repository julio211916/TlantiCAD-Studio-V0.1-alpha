  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        'after:absolute after:-inset-2 after:md:hidden',
        'group-data-[collapsible=icon]:hidden',
        showOnHover && 'md:opacity-0 group-focus-within/menu-item:opacity-100 group-hover/menu-item:opacity-100',
        className,
      )}
      {...props}
    />
...
function SidebarBody({ children, className }: { children: React.ReactNode; className?: string }) {
  return <div className={cn('flex min-h-0 flex-1 flex-col', className)}>{children}</div>;
}

function SidebarLink({ link, className }: { link: SidebarLinkItem; className?: string }) {
  const content = (
    <>
      {link.icon}
  ...
  if (link.onClick || !link.href) {
    return (
      <SidebarMenuButton type="button" onClick={link.onClick} className={className}>
        {content}
      </SidebarMenuButton>
    );
  }
