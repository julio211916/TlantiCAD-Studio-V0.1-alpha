 ...
 * This is a capability-gated legacy adapter. It is unavailable by default
 * because production TlantiCAD must route DICOM/IA through local Tauri/Python
 * runtime checks, not an implicit generic HTTP backend.
 */
...
const DEFAULT_LOCAL_BACKEND_ORIGIN = 'http://127.0.0.1:17493';
...
export function createBackendSegmentationAdapter(options?: {
    ...
    /**
     * Must be true before launch/poll/cancel can use HTTP. Default false keeps
     * generic adapters visible but unavailable until a local sidecar capability
     * explicitly owns the job lifecycle.
     */
    enabled?: boolean;
}): SegmentationBackendPort {
    const origin = options?.origin ?? DEFAULT_LOCAL_BACKEND_ORIGIN;
    const fetcher = options?.fetcher ?? fetch;

    function assertCapability() {
        if (options?.enabled !== true) {
            throw new Error(
                'DICOM segmentation HTTP adapter is unavailable. Use the local Tauri/Python runtime path or explicitly enable the loopback adapter for a supervised local sidecar.',
            );
        }
        const url = new URL(origin);
        if (url.protocol !== 'http:') {
            throw new Error(`DICOM segmentation adapter only permits local http loopback, received ${url.protocol}`);
        }
        if (!['127.0.0.1', 'localhost', '[::1]', '::1'].includes(url.hostname)) {
            throw new Error(`DICOM segmentation adapter blocked non-loopback host: ${url.hostname}`);
        }
    }
    ...
    return {
        ...
        async cancel(jobId) {
            await postJson<{ ok: boolean }>(
                `/clinical-vendors/jobs/${encodeURIComponent(jobId)}/cancel`,
                {},
            );
        },
    };
