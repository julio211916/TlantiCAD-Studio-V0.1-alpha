...
import { useCadSceneStore, type CadSceneEntity } from '@/features/cad-scene';
...
function useFixtureMaterial(selected: boolean) {
  const material = useMemo(() => new THREE.MeshStandardMaterial({
    color: '#d8e2dd',
    metalness: 0.05,
    roughness: 0.42,
    emissive: selected ? '#155e75' : '#000000',
...
const FixtureMesh = memo(function FixtureMesh({
  ...
  const material = useFixtureMaterial(selected);
...
function CadSceneContent() {
  ...
  const invalidate = useThree((state) => state.invalidate);
  const meshEntity = entities.find((entity) => entity.id === 'fixture-upper-scan');
  ...
  return (
    <>
      ...
      {meshEntity && <FixtureMesh entity={meshEntity} selected={selectedEntityId === meshEntity.id} onSelect={selectEntity} />}
...
export function CadViewportShell() {
  ...
  return (
    <Canvas
      ...
      camera={{ position: [4.5, 3.2, 5.2], fov: 42, near: 0.05, far: 1000 }}
